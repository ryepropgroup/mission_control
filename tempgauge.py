...
class LinearGaugeTemp(QWidget):
    ...
    def paintEvent(self, event):
        ...
        for i in range(len(self.value_colors) - 1):
            ...
            start_pos = int(gauge_rect.left() + gauge_rect.width() * (start_threshold - self.min_value) / total_range)
            end_pos = int(gauge_rect.left() + gauge_rect.width() * (end_threshold - self.min_value) / total_range)
        ...
        dial_pos = int(gauge_rect.left() + gauge_rect.width() * (self.value - self.min_value) / total_range)
